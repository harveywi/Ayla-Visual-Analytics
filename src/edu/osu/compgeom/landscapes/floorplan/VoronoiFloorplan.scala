...
import edu.osu.compgeom.util.Looping
...
object VoronoiFloorplan {
  ...
  def apply(
    ...
    val connected1 = new ConnectivityInspector(g).isGraphConnected()

    Timing("updating terrain graph") {
      val mmap = new scala.collection.mutable.HashMap[(Float, Float, Float), scala.collection.mutable.ArrayBuffer[Int]]
      vertices.zipWithIndex.foreach {
        case (v, i) => {
          val list = mmap.getOrElseUpdate((v.x, v.y, v.z), new scala.collection.mutable.ArrayBuffer[Int])
          list += i
        }
      }
    ...
    val connected2 = new ConnectivityInspector(g).isGraphConnected()
    println("Connected1?  " + connected1)
    println("Connected2?  " + connected2)
    
//    // There should be a path between two vertices
//    import org.jgrapht.alg.DijkstraShortestPath
//    
//    (0 until entireIndexedTriArray.getVertexCount()).combinations(2).foreach{case Seq(i, j) => 
//      val dsp = new DijkstraShortestPath(g, i, j)
//      println("Path length:  " + dsp.getPathLength())
//      println("\n")
//    }
//    System.exit(0)
