package edu.osu.compgeom.omegavis.state

import java.io._
import scala.swing.Publisher
import scala.swing.Reactor
import edu.osu.compgeom.dataset.Dataset
import edu.osu.compgeom.omegavis.RingMenuItem
...
import edu.osu.compgeom.ayla.AylaClientCachedDataset
...
class StateManager(val dataset: Dataset, val morseFunction: ScalarFunction, val ctSimp: ContourTree,
  ...
  var colormapGenerator: ScalarFunction => GammaColormap = getJetColormap _
...
//      val colorFuncVals = lines.drop(1).map(_.toFloat).toArray
      val colorFuncVals = dataset.getColorFunction(file)
  ...
  */
  ...
  def getJetColormap(g: ScalarFunction): GammaColormap = new JetGammaColormap(g)

  def getDivergingRedBlueColormap(g: ScalarFunction) = getDivergingColormap(g, Color.red, Color.blue)

  def getDivergingColormap(g: ScalarFunction, colorLow: Color, colorHigh: Color): GammaColormap = {
    val cmInterp = new edu.osu.compgeom.color.DivergingColormap(colorLow, colorHigh)
    val gammaCmap = new DivergingGammaColormap(g, cmInterp)
    gammaCmap
  }
  ...
  val menuBinaryColormap = new RingMenuItem("Binary") {
    override def handleClick(progressListener: RingMenuProgressListener) = {
      ...
      val gammaCmap = new ProteinGammaColormap(minCount, maxCount, roi, colorFunction)
...
//            dataset match {
//              case ds: AylaClientCachedDataset with FilePdbStreamProvider => {
//              	ds.sampledToUnsampled
//              }
//              case provider: ZipFilePdbStreamProvider => {}
//            }
//            regex.pattern.matcher("blah").matches()
          }
  ...
  */
  ...
  def awaitTauOrColorFuncMenu = {
    ...
    dataset.colorFunctions.foreach {
      case (funcFile, funcName) =>
  ...
  def showBinaryFunction(f: ScalarFunction, gFuncVals: Array[Float], ctSimp: ContourTree): Unit = {
    val g = new ScalarFunction(f.vertices, f.faces, gFuncVals)
      ...
      val minCount = counts.min
      val maxCount = counts.max
      val gammaCmap = new ProteinGammaColormap(minCount, maxCount, roi, g)
  ...
  def showSecondaryStructure(f: ScalarFunction, gFuncVals: Array[Float], ctSimp: ContourTree): Unit = {
    ...
    val (gammaCmap, statisticSF) = topoComponentColorMode match {
      ...
      case Mean => {
        ...
        val cmap = new GammaColormap(meanSF) {
...
//        }, meanSF)
      ...
      case LocalCorrelation => {
        ...
        (new DivergingGammaColormap(stdevSF, cmInterp) {
