...
public class NearestNeighborSearcher {
	...
	public NearestNeighborSearcher(float[][] pts) {
		...
		for (int idx = 0; idx < pts.length; ++idx) {
			...
			// stdev2[idx][j] = getStdev(mu2[idx][j], idx, mod2*j,
			// Math.min(mod2*(j+1), pts.length));
			...
			for (int j = 0; j < 4; ++j) {
				...
				stdev4[idx][j] = getStdev(mu4[idx][j], idx, mod4 * j,
						Math.min(mod4 * (j + 1), nDims));
			...
			for (int j = 0; j < 16; ++j) {
				...
				stdev16[idx][j] = getStdev(mu16[idx][j], idx, mod16 * j,
						Math.min(mod16 * (j + 1), nDims));
	...
	/**
	 * Returns the k-nearest neighbors of the point with index y. The list of
	 * neighbors is guaranteed to not contain y.
	 * 
	 * @param y
	 *          Index of point whose neighbors you wish to find
	 * @param k
	 *          Number of nearest neighbors of y to find.
	...
	public int[] getNearestNeighbors(int y, int k) {
		...
		if (nDims < 16) {
			...
			for (int i = 0; i < pts.length; i++) {
				...
				double distSq = euclidDistSq(i, y, 0, nDims);
				if (distSq < minDist) {
					pq.add(new DistPair(i, distSq));
					if (pq.size() > k + 1)
						pq.poll();
				}
			}
			...
			return getNearestNeighbors_HighDimInput(y, k);
		}
	}

	public int[] getNearestNeighbors_HighDimInput(int y, int k) {
		...
		for (int x = 0; x < pts.length; ++x) {
			...
			for (int i = 0; i < 16; ++i) {
				...
				bnd += euclidDistSq(x, y, from, until)
						- lb(d16[i], mu16[x][i], stdev16[x][i], mu16[y][i], stdev16[y][i]);
				if (bnd >= minDist) {
					break;
				}
			}
			...
			// double check2 = 0;
			// for (int i = 0; i < 16; i++) {
			// check2 += lb(d16[x][i], mu16[x][i], stdev16[x][i], mu16[y][i],
			// stdev16[y][i]);
			// }
			//
	...
	private final double lb(int d, double mean1, double stdev1, double mean2,
			double stdev2) {
