...
class PieMenu(graph: SimpleDirectedGraph[PieMenuItem, DefaultEdge]) extends Component {
...
object PieMenuTest extends SimpleSwingApplication {
  ...
  while (!stack.isEmpty) {
    ...
    val childNodes = (n.userData match {
      ...
      case Some(_) => throw new UnsupportedOperationException("userData must be a file.")
