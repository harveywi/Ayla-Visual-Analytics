...
class OpenProjectDialog(menu: PieMenu) extends Dialog {
  menu.mainFrame = this
  var chosenProject: Option[OpenProjectDialogData] = None
  ...
  reactions += {
    case UIElementHidden(`menu`) => {
      chosenProject = menu.selectedItem match {
        case Some(menuItem) => menuItem.userData.asInstanceOf[Option[OpenProjectDialogData]]
        case _ => None
      }
      close()
    }
...
case class OpenProjectDialogData(datasetName: String, projName: String, scalarFunctionName: String) extends Serializable

object OpenProjectDialog {
  def chooseProject(menu: PieMenu): Option[OpenProjectDialogData] = {
